// src/pages/admin/ShiftApprovalCalendar.tsx
...
const ShiftApprovalCalendar: React.FC = () => {
  // State for pending shifts
  const [pendingRequests, setPendingRequests] = useState<any[]>([]);
  // State for approved shifts
  const [approvedShifts, setApprovedShifts] = useState<any[]>([]);
    ...
    const getShifts = async () => {
      ...
      if (shifts.length > 0) {
        const formattedShifts = shifts.map((shift: any) => ({
          id: shift.id,
          employee: `Employee ${shift.employeeId}`, // Replace with actual name if available
        ...
        const pending = formattedShifts.filter(
          (shift: any) => shift.status === "PROPOSED"
        );
        const approved = formattedShifts.filter(
          (shift: any) => shift.status === "APPROVED"
        );
    ...
    try {
      ...
      if (approvedShift.status === "ACCEPTED") {
        ...
        setApprovedShifts((prevApproved) => [
          ...
          {
            id: approvedShift.id,
            employee: `Employee ${approvedShift.employeeId}`,
  ...
  // Custom event style for calendar events
